package brys.org.dev.Paginator.listener;
...
public class MessageHandler extends ListenerAdapter {
    ...
    public void removeEvent(Message msg) {
        switch (msg.getChannelType()) {
            case TEXT -> events.remove(msg.getGuild().getId() + msg.getId());
            case PRIVATE -> events.remove(msg.getPrivateChannel().getId() + msg.getId());
        }
    }
    ...
    public void onMessageDelete(@Nonnull MessageDeleteEvent evt) {
        switch (evt.getChannelType()) {
            case TEXT -> events.remove(evt.getGuild().getId() + evt.getMessageId());
            case PRIVATE -> events.remove(evt.getPrivateChannel().getId() + evt.getMessageId());
        }
    }
